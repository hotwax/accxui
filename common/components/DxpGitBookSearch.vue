<template>
  ...
  <ion-modal ref="gitBookSearchModal" trigger="gibook-search-modal" @willDismiss="clearQueryState()">
    ...
    <ion-content>
      <ion-searchbar :placeholder="translate(selectedSegment === 'search' ? 'Search...': 'Ask...')" :value="queryString" @keyup.enter="queryString = $event.target.value; selectedSegment === 'search' ? gitBookSearchQuery() : gitBookAskQuery()" />
...
async function gitBookAskQuery() {
  ...
  try {
    const resp = await gitBookApi.askQuery({ 
      ...
      token: import.meta.env.VITE_VUE_APP_GITBOOK_API_KEY
    });
...
async function gitBookSearchQuery() {
  isLoading.value = true;
  let items = [] as any;
